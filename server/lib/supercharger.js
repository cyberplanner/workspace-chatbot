...
const execute = (session, args, next, conversationDoc, skip) => {
    ...
    .map(key => {
      ...
      if (value.indexOf("$") == 0) {
        ...
        if (
          ...
          session.userData.summary[value.substring(1)]
        ) {
          return {
            key: key,
          ...
          return {
            key: key,
            value: botBuilder.EntityRecognizer.findEntity(
              args.intent.entities,
              value.substring(1)
            )
          };
  ...
  logger.debug(
    "[SUPERCHARGER] Retrieving supercharger with ID: " +
      conversationDoc.supercharger.id
  );
